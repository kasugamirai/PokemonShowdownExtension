...
},{"./translate":3}],2:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.translateElement = exports.t = exports.trans_from_dict = void 0;
const translations_1 = require("./translations");
const regex_1 = require("./regex");
function trans_from_dict(a) {
    var b = translations_1.translations[a];
    if (b)
        return b;
    return a;
}
exports.trans_from_dict = trans_from_dict;
function t(originalStr, translations) {
    var tmp = originalStr.trim();
    if (translations[tmp])
        return translations[tmp];
    if (originalStr.match(regex_1.regex.regex_chn))
        return originalStr;
    if (originalStr.match(regex_1.regex.regex_team)) {
        return RegExp.$1 + "的队伍：";
    }
    if (originalStr.match(regex_1.regex.regex_youjoined)) {
        return "你加入了" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_seconds_left)) {
        return RegExp.$1 + "还剩" + RegExp.$2 + "秒。";
    }
    if (originalStr.match(regex_1.regex.regex_timer_on)) {
        return "战斗计时器已经开启：玩家若不行动则在时间耗尽后会输掉比赛。（由" + RegExp.$1 + "发起）";
    }
    if (originalStr.match(regex_1.regex.regex_reset_timer)) {
        return "还剩" + RegExp.$1 + "秒可以重新开启计时器。";
    }
    if (originalStr.match(regex_1.regex.regex_joined)) {
        return RegExp.$1.replace(", ", "，").replace(/ and /g, "和") + "进入了房间";
    }
    if (originalStr.match(regex_1.regex.regex_left)) {
        return RegExp.$1.replace(", ", "，").replace(/ and /g, "和") + "离开了";
    }
    if (originalStr.match(regex_1.regex.regex_sent_out)) {
        return RegExp.$1 + "放出了";
    }
    if (originalStr.match(regex_1.regex.regex_lost_health)) {
        return trans_from_dict(RegExp.$1) + trans_from_dict(RegExp.$2) + "失去了 " + RegExp.$3 + "% 的生命值！";
    }
    if (originalStr.match(regex_1.regex.regex_lost_health2)) {
        return trans_from_dict(RegExp.$1) + trans_from_dict(RegExp.$2) + "失去了 ";
    }
    if (originalStr.match(regex_1.regex.regex_hp_restored)) {
    ...
    if (originalStr.match(regex_1.regex.regex_come_back)) {
        return trans_from_dict(RegExp.$1) + "，回来吧！";
    }
    if (originalStr.match(regex_1.regex.regex_go)) {
        return "去吧，" + trans_from_dict(RegExp.$1) + "(";
    }
    if (originalStr.match(regex_1.regex.regex_stat_change)) {
        if (trans_from_dict(RegExp.$3) != RegExp.$3)
            return trans_from_dict(RegExp.$1) + trans_from_dict(RegExp.$2) + trans_from_dict(RegExp.$3);
    }
    if (originalStr.match(regex_1.regex.regex_forfeited)) {
    ...
    if (originalStr.match(regex_1.regex.regex_trace)) {
    ...
    if (originalStr.match(regex_1.regex.regex_Item)) {
        originalStr = originalStr.replace(regex_1.regex.regex_Item, "道具: ");
        if (translations[RegExp.$1]) {
            originalStr += translations[RegExp.$1];
        }
        return originalStr;
    }
    if (originalStr.match(regex_1.regex.regex_start_battle)) {
    ...
    if (originalStr.match(regex_1.regex.regex_uturn)) {
        return trans_from_dict(RegExp.$1) + trans_from_dict(RegExp.$2) + "回到了" + RegExp.$3 + "的身边！";
    }
    if (originalStr.match(regex_1.regex.regex_magic_bounce)) {
        return "把" + translations[RegExp.$1] + "反弹回去了！";
    }
    if (originalStr.match(regex_1.regex.regex_dynamax)) {
        return trans_from_dict(RegExp.$1) + trans_from_dict(RegExp.$2) + "的极巨化！";
    }
    if (originalStr.match(regex_1.regex.regex_gems)) {
        return translations[RegExp.$1] + "提升了" + translations[RegExp.$2] + "的威力!";
    }
    if (originalStr.match(regex_1.regex.regex_room_expired)) {
        return '房间“' + RegExp.$1 + '”不存在。\n您要进入的对战已过期。超过15分钟未活动且未保存的对战将过期，请及时保存回放。';
    }
    if (originalStr.match(regex_1.regex.regex_battle)) {
        return RegExp.$1 + "想要战斗！";
    }
    if (originalStr.match(regex_1.regex.regex_cancelled)) {
        return RegExp.$1 + "取消了战斗。";
    }
    if (originalStr.match(regex_1.regex.regex_wftcy)) {
        return "等待" + RegExp.$1 + "挑战你";
    }
    if (originalStr.match(regex_1.regex.regex_waitingavailable)) {
        return "等待战斗开始" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_waiting)) {
        return "等待" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_accepted)) {
        return RegExp.$1 + "接受挑战，对战开始";
    }
    if (originalStr.match(regex_1.regex.regex_copyofuntitled)) {
        return "副本无标题" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_untitled)) {
        return "无标题" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_copyof)) {
        return "副本" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_newteam)) {
        return "新的" + RegExp.$1 + "队伍";
    }
    if (originalStr.match(regex_1.regex.regex_users)) {
        return RegExp.$1 + "位用户";
    }
    if (originalStr.match(regex_1.regex.regex_theopposingfainted)) {
        return "对手的" + translations[RegExp.$1] + "倒下了！";
    }
    if (originalStr.match(regex_1.regex.regex_fainted)) {
        return translations[RegExp.$1] + "倒下了！";
    }
    if (originalStr.match(regex_1.regex.regex_theopposingleftover)) {
        return "对手的" + translations[RegExp.$1] + "通过剩饭恢复了一点HP！";
    }
    if (originalStr.match(regex_1.regex.regex_leftover)) {
        return translations[RegExp.$1] + "通过剩饭恢复了一点HP！";
    }
    if (originalStr.match(regex_1.regex.regex_wish)) {
        return translations[RegExp.$1] + "的祈愿成真了！";
    }
    if (originalStr.match(regex_1.regex.regex_doestaffecttd)) {
        return "这并没有影响到对手的" + translations[RegExp.$1];
    }
    if (originalStr.match(regex_1.regex.regex_doestaffect)) {
        return "这并没有影响到" + translations[RegExp.$1];
    }
    if (originalStr.match(regex_1.regex.regex_younoteams)) {
        return "你没有" + RegExp.$1 + "队伍";
    }
    if (originalStr.match(regex_1.regex.regex_youdontha)) {
        return "你没有任何" + RegExp.$1 + "队伍";
    }
    if (originalStr.match(regex_1.regex.regex_xteams)) {
        return RegExp.$1 + "队伍";
    }
    if (originalStr.match(regex_1.regex.regex_theinverted)) {
        return "对手的" + translations[RegExp.$1] + "能力变化颠倒过来了！";
    }
    if (originalStr.match(regex_1.regex.regex_inverted)) {
        return translations[RegExp.$1] + "能力变化颠倒过来了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackrd)) {
        return "对手" + translations[RegExp.$1] + "的攻击巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefenserd)) {
        return "对手" + translations[RegExp.$1] + "的防御巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospard)) {
        return "对手" + translations[RegExp.$1] + "的特攻巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdrd)) {
        return "对手" + translations[RegExp.$1] + "的特防巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tosperd)) {
        return "对手" + translations[RegExp.$1] + "的速度巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessrd)) {
        return "对手" + translations[RegExp.$1] + "的闪避率巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyrd)) {
        return "对手" + translations[RegExp.$1] + "的命中率巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackrd)) {
        return translations[RegExp.$1] + "的攻击巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_defenserd)) {
        return translations[RegExp.$1] + "的防御巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spard)) {
        return translations[RegExp.$1] + "的特攻巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdrd)) {
        return translations[RegExp.$1] + "的特防巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_sperd)) {
        return translations[RegExp.$1] + "的速度巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessrd)) {
        return translations[RegExp.$1] + "的闪避率巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyrd)) {
        return translations[RegExp.$1] + "的命中率巨幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackfs)) {
        return "对手" + translations[RegExp.$1] + "的攻击巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefensefs)) {
        return "对手" + translations[RegExp.$1] + "的防御巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospafs)) {
        return "对手" + translations[RegExp.$1] + "的特攻巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdfs)) {
        return "对手" + translations[RegExp.$1] + "的特防巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospefs)) {
        return "对手" + translations[RegExp.$1] + "的速度巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessfs)) {
        return "对手" + translations[RegExp.$1] + "的闪避率巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyfs)) {
        return "对手" + translations[RegExp.$1] + "的命中率巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackrfs)) {
        return translations[RegExp.$1] + "的攻击巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_defensefs)) {
        return translations[RegExp.$1] + "的防御巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spafs)) {
        return translations[RegExp.$1] + "的特攻巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdfs)) {
        return translations[RegExp.$1] + "的特防巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spefs)) {
        return translations[RegExp.$1] + "的速度巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessfs)) {
        return translations[RegExp.$1] + "的闪避率巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyfs)) {
        return translations[RegExp.$1] + "的命中率巨幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的攻击大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefenserosesharply)) {
        return "对手" + translations[RegExp.$1] + "的防御大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospatkrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的特攻大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdefrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的特防大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospeedrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的速度大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的闪避率大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyrosesharply)) {
        return "对手" + translations[RegExp.$1] + "的命中率大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackfellharshly)) {
        return "对手" + translations[RegExp.$1] + "的攻击大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefensefellharshly)) {
        return "对手" + translations[RegExp.$1] + "的防御大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospatkfellharshly)) {
        return "对手" + translations[RegExp.$1] + "的特攻大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdeffellharshly)) {
        return "对手" + translations[RegExp.$1] + "的特防大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospeedfellharshly)) {
        return "对手" + translations[RegExp.$1] + "的速度大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessfellharshly)) {
        return "对手" + translations[RegExp.$1] + "的闪避率大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyfellharshly)) {
        return "对手" + translations[RegExp.$1] + "的命中率大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackrose)) {
        return "对手" + translations[RegExp.$1] + "的攻击提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefenserose)) {
        return "对手" + translations[RegExp.$1] + "的防御提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospatkrose)) {
        return "对手" + translations[RegExp.$1] + "的特攻提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdefrose)) {
        return "对手" + translations[RegExp.$1] + "的特防提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospeedrose)) {
        return "对手" + translations[RegExp.$1] + "的速度提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessrose)) {
        return "对手" + translations[RegExp.$1] + "的闪避率提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyrose)) {
        return "对手" + translations[RegExp.$1] + "的命中率提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackfell)) {
        return "对手" + translations[RegExp.$1] + "的攻击降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefensefell)) {
        return "对手" + translations[RegExp.$1] + "的防御降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospatkfell)) {
        return "对手" + translations[RegExp.$1] + "的特攻降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdeffell)) {
        return "对手" + translations[RegExp.$1] + "的特防降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospeedfell)) {
        return "对手" + translations[RegExp.$1] + "的速度降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivenessfell)) {
        return "对手" + translations[RegExp.$1] + "的闪避率降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyfell)) {
        return "对手" + translations[RegExp.$1] + "的命中率降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackrosesharply)) {
        return translations[RegExp.$1] + "的攻击大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_defenserosesharply)) {
        return translations[RegExp.$1] + "的防御大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spatkrosesharply)) {
        return translations[RegExp.$1] + "的特攻大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdefrosesharply)) {
        return translations[RegExp.$1] + "的特防大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_speedrosesharply)) {
        return translations[RegExp.$1] + "的速度大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessrosesharply)) {
        return translations[RegExp.$1] + "的闪避率大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyrosesharply)) {
        return translations[RegExp.$1] + "的命中率大幅提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackfellharshly)) {
        return translations[RegExp.$1] + "的攻击大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_defensefellharshly)) {
        return translations[RegExp.$1] + "的防御大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spatkfellharshly)) {
        return translations[RegExp.$1] + "的特攻大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdeffellharshly)) {
        return translations[RegExp.$1] + "的特防大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_speedfellharshly)) {
        return translations[RegExp.$1] + "的速度大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessfellharshly)) {
        return translations[RegExp.$1] + "的闪避率大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyfellharshly)) {
        return translations[RegExp.$1] + "的命中率大幅降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackrose)) {
        return translations[RegExp.$1] + "的攻击提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_defenserose)) {
        return translations[RegExp.$1] + "的防御提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spatkrose)) {
        return translations[RegExp.$1] + "的特攻提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdefrose)) {
        return translations[RegExp.$1] + "的特防提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_speedrose)) {
        return translations[RegExp.$1] + "的速度提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessrose)) {
        return translations[RegExp.$1] + "的闪避率提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyrose)) {
        return translations[RegExp.$1] + "的命中率提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackfell)) {
        return translations[RegExp.$1] + "的攻击降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_defensefell)) {
        return translations[RegExp.$1] + "的防御降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spatkfell)) {
        return translations[RegExp.$1] + "的特攻降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdeffell)) {
        return translations[RegExp.$1] + "的特防降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_speedfell)) {
        return translations[RegExp.$1] + "的速度降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivenessfell)) {
        return translations[RegExp.$1] + "的闪避率降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyfell)) {
        return translations[RegExp.$1] + "的命中率降低了！";
    }
    if (originalStr.match(regex_1.regex.regex_toattackh)) {
        return "对手" + translations[RegExp.$1] + "的攻击已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_todefnseh)) {
        return "对手" + translations[RegExp.$1] + "的防御已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospah)) {
        return "对手" + translations[RegExp.$1] + "的特攻已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospdh)) {
        return "对手" + translations[RegExp.$1] + "的特防已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_tospeh)) {
        return "对手" + translations[RegExp.$1] + "的速度已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toevasivengessh)) {
        return "对手" + translations[RegExp.$1] + "的闪避率已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_toaccuracyh)) {
        return "对手" + translations[RegExp.$1] + "的命中率已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_attackh)) {
        return translations[RegExp.$1] + "的攻击已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_defnseh)) {
        return translations[RegExp.$1] + "的防御已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spah)) {
        return translations[RegExp.$1] + "的特攻已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_spdh)) {
        return translations[RegExp.$1] + "的特防已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_speh)) {
        return translations[RegExp.$1] + "的速度已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_evasivengessh)) {
        return translations[RegExp.$1] + "的闪避率已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_accuracyh)) {
        return translations[RegExp.$1] + "的命中率已经无法再提高了！";
    }
    if (originalStr.match(regex_1.regex.regex_rejectchallenge)) {
        return RegExp.$1 + "拒绝了挑战。";
    }
    if (originalStr.match(regex_1.regex.regex_thesustookto)) {
        return "替身承受了伤害保护了对手的" + translations[RegExp.$1];
    }
    if (originalStr.match(regex_1.regex.regex_thesustook)) {
        return "替身承受了伤害保护了" + translations[RegExp.$1];
    }
    if (originalStr.match(regex_1.regex.regex_tohbawi)) {
        return "对手的" + translations[RegExp.$1] + "受到了来自" + translations[RegExp.$2] + "死缠烂打的折磨";
    }
    if (originalStr.match(regex_1.regex.regex_hbawi)) {
        return translations[RegExp.$1] + "受到了来自对手" + translations[RegExp.$2] + "死缠烂打的折磨";
    }
    if (originalStr.match(regex_1.regex.regex_toihb)) {
        return "对手的" + translations[RegExp.$1] + "受到了" + translations[RegExp.$2] + "的伤害";
    }
    if (originalStr.match(regex_1.regex.regex_ihb)) {
        return translations[RegExp.$1] + "受到了" + translations[RegExp.$2] + "的伤害";
    }
    if (originalStr.match(regex_1.regex.regex_iseoto)) {
        return "这对对手的" + translations[RegExp.$1] + "非常有效";
    }
    if (originalStr.match(regex_1.regex.regex_iseo)) {
        return "这对" + translations[RegExp.$1] + "非常有效";
    }
    if (originalStr.match(regex_1.regex.regex_isnveoto)) {
        return "这对对手的" + translations[RegExp.$1] + "不是很有效";
    }
    if (originalStr.match(regex_1.regex.regex_isnveo)) {
        return "这对" + translations[RegExp.$1] + "不是很有效";
    }
    if (originalStr.match(regex_1.regex.regex_achoto)) {
        return "击中了对手" + translations[RegExp.$1] + "的要害";
    }
    if (originalStr.match(regex_1.regex.regex_acho)) {
        return "击中了" + translations[RegExp.$1] + "的要害";
    }
    if (originalStr.match(regex_1.regex.regex_setc)) {
        return RegExp.$1 + "单败淘汰赛";
    }
    if (originalStr.match(regex_1.regex.regex_willuse)) {
        return RegExp.$1 + "将使用" + RegExp.$2;
    }
    if (originalStr.match(regex_1.regex.regex_willswitchin)) {
        return RegExp.$1 + "将替换" + RegExp.$2 + "上场";
    }
    if (originalStr.match(regex_1.regex.regex_challengex)) {
        return "挑战" + RegExp.$1;
    }
    if (originalStr.match(regex_1.regex.regex_uteamsvf)) {
        return "您的队伍在" + RegExp.$1 + "分级可以合法使用";
    }
    if (originalStr.match(regex_1.regex.regex_Metronome)) {
        return "摇手指让它使出了" + translations[RegExp.$1] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_toiatbabi)) {
        return "对手的" + translations[RegExp.$1] + "受到了来自它" + translations[RegExp.$2] + "的攻击！";
    }
    if (originalStr.match(regex_1.regex.regex_iatbabi)) {
        return translations[RegExp.$1] + "受到了来自它" + translations[RegExp.$2] + "的攻击！";
    }
    if (originalStr.match(regex_1.regex.regex_toctop)) {
        return "对手的" + translations[RegExp.$1] + "腐蚀了" + translations[RegExp.$2] + "的" + translations[RegExp.$3];
    }
    if (originalStr.match(regex_1.regex.regex_ctop)) {
        return translations[RegExp.$1] + "腐蚀了对手" + translations[RegExp.$2] + "的" + translations[RegExp.$3];
    }
    if (originalStr.match(regex_1.regex.regex_biftato)) {
        return "但没能影响到对手的" + translations[RegExp.$1] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_bifta)) {
        return "但没能影响到" + translations[RegExp.$1] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_toshpif)) {
        return "对手的" + translations[RegExp.$1] + "HP已满！";
    }
    if (originalStr.match(regex_1.regex.regex_shpif)) {
        return translations[RegExp.$1] + "HP已满！";
    }
    if (originalStr.match(regex_1.regex.regex_tobiuiz)) {
        return "对手的" + translations[RegExp.$1] + "利用Z力量强化了自身的" + translations[RegExp.$2] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_biuiz)) {
        return translations[RegExp.$1] + "利用Z力量强化了自身的" + translations[RegExp.$2] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_tobisdizp)) {
        return "对手的" + translations[RegExp.$1] + "利用Z力量强化了自身的特防！";
    }
    if (originalStr.match(regex_1.regex.regex_tobisaizp)) {
        return "对手的" + translations[RegExp.$1] + "利用Z力量强化了自身的特攻！";
    }
    if (originalStr.match(regex_1.regex.regex_bisdizp)) {
        return translations[RegExp.$1] + "利用Z力量强化了自身的特防！";
    }
    if (originalStr.match(regex_1.regex.regex_bisaizp)) {
        return translations[RegExp.$1] + "利用Z力量强化了自身的特攻！";
    }
    if (originalStr.match(regex_1.regex.regex_tobstct)) {
        return "对手的" + translations[RegExp.$1] + "的属性转变成了" + translations[RegExp.$2] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_bstct)) {
        return translations[RegExp.$1] + "的属性转变成了" + translations[RegExp.$2] + "！";
    }
    if (originalStr.match(regex_1.regex.regex_towhbisb)) {
        return "对手的" + translations[RegExp.$1] + "受到了附着针的伤害！";
    }
    if (originalStr.match(regex_1.regex.regex_whbisb)) {
        return translations[RegExp.$1] + "受到了附着针的伤害！";
    }
    if (originalStr.match(regex_1.regex.regex_twpsrtosa)) {
        return "弱点保险大幅提高了对手" + translations[RegExp.$1] + "的攻击！";
    }
    if (originalStr.match(regex_1.regex.regex_twpsrtossa)) {
        return "弱点保险大幅提高了对手" + translations[RegExp.$1] + "的特攻！";
    }
    if (originalStr.match(regex_1.regex.regex_twpsrsa)) {
        return "弱点保险大幅提高" + translations[RegExp.$1] + "的攻击！";
    }
    if (originalStr.match(regex_1.regex.regex_twpsrssa)) {
        return "弱点保险大幅提高了" + translations[RegExp.$1] + "的特攻！";
    }
    if (originalStr.match(regex_1.regex.regex_thwctfto)) {
        return "对手" + translations[RegExp.$1] + "的治愈之愿实现了！";
    }
    if (originalStr.match(regex_1.regex.regex_thwctf)) {
        return translations[RegExp.$1] + "的治愈之愿实现了！";
    }
    if (originalStr.match(regex_1.regex.regex_psdito)) {
        return "锋利的岩石扎进了对手的" + RegExp.$1 + "的身体";
    }
    if (originalStr.match(regex_1.regex.regex_psdi)) {
        return "锋利的岩石扎进了" + RegExp.$1 + "的身体";
    }
    if (originalStr.match(regex_1.regex.regex_sfwhrrm)) {
        return RegExp.$1 + "衷心的祈愿传递到了烈空坐那里！";
    }
    if (originalStr.match(regex_1.regex.regex_sfwhrtorm)) {
...
function translateElement(element) {
    ...
    while ((node = elTW.nextNode()) != null) {
        ...
        if (node.parentNode) {
            console.log('Parent node tag name:', node.parentNode.tagName);
        }
        ...
        if (node.parentNode && node.parentNode.tagName === "STRONG" ||
            node.parentNode && node.parentNode.getAttribute("class") === "col movenamecol" ||
            node.parentNode && node.parentNode.parentNode && node.parentNode.parentNode.getAttribute("class") === "col movenamecol" ||
            node.parentNode && node.parentNode.getAttribute("name") === "chooseMove") {
        ...
        if (value && node.nodeValue?.indexOf('•') !== -1) {
            ...
            value = translate(value, translations_1.translations);
            node.nodeValue = "• " + value;
            console.log('Translated text:', node.nodeValue);
            continue;
        }
        if (node.nodeValue) {
            node.nodeValue = translate(node.nodeValue.replace("é", "e"), translations_1.translations);
            console.log('Translated text:', node.nodeValue);
        }
    }
}
...
},{"./regex":2,"./translations":4}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
